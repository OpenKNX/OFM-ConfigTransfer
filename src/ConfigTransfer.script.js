...
// TODO move
function uctProgress(progress, pos) {
    if (progress) {
        progress.setProgress(pos);
    }
}
function uctProgressCalc(progress, a, b, current, max) {
    if (progress) {
        progress.setProgress(a + (b-a)*current/max);
    }
}
    ...
    if (moduleSelection < uctModuleOrder.length) {
        ...
        for (var i = 0; i < channels.length; i++) {
            if (!uctProgressIsCanceled(progress)) {
                ...
                uctProgressCalc(progress, 2, 97, i, channels.length);
        ...
        for (var i = 0; i < uctModuleOrder.length; i++) {
            ...
            for (var channelNumber = 0; channelNumber <= moduleChannelCount; channelNumber++) {
                if (uctProgressIsCanceled(progress)) {
                    ...
                    uctProgressCalc(progress, 1, 97, progressPos, progressMax);
                }
            }
        }
        param_exportOutput.value = channelExportResult.join("\n");
    ...
    if (mode == 0) {
        ...
        for (var i = 0; i < sourceChannels.length; i++) {
            if (!uctProgressIsCanceled(progress)) {
                ...
                uctProgressText(progress, progressText + " ...");
                uctProgressCalc(progress, 2, 97, i, sourceChannels.length);
...
function uctWriteParams(device, module, channel, params, newValues, result) {
    ...
    for (var i = 0; i < params.names.length; i++) {
        ...
        try {
            ...
            var objErr = ("" + e + "") == "[object Error]";
            if (objErr) {
                result.lines.push("* { ! } " + paramFullNameTempl + "=" + paramValue + "  \t" + e.message);
            } else {
                // TODO remove/replace completely
